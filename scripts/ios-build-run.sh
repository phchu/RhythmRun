...
xcodebuild \
  ...
  build | xcpretty || xcodebuild -project "$PROJECT_PATH" -scheme "$SCHEME" -configuration "$CONFIGURATION" -sdk "$SDK" -destination "generic/platform=iOS Simulator" build
...
if [ -z "$DEVICE_ID" ]; then
    echo "⚠️ No booted simulator found. Booting default iPhone..."
    DEVICE_ID=$(xcrun simctl list devices | grep "iPhone" | grep -v "unavailable" | head -n 1 | grep -E '([0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12})' -o)
...
APP_PATH=$(xcodebuild -project "$PROJECT_PATH" -scheme "$SCHEME" -configuration "$CONFIGURATION" -sdk "$SDK" -showBuildSettings | grep -w TARGET_BUILD_DIR | awk '{print $3}')
